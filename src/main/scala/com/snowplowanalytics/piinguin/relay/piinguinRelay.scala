...
class PiinguinRelay {
  ...
  def recordHandler(event: KinesisEvent): Unit = {
    ...
    if (errorList.nonEmpty) throw new PiinguinRelayRuntimeException(s"Errors encountered ${errorList.mkString("\n")}")
  }
...
class PiinguinRelayRuntimeException(reason: String) extends RuntimeException(reason)
